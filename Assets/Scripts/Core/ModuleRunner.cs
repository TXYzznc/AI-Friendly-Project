...
public class ModuleRunner
{
    ...
    public async UniTask StartAsync()
    {
        ...
        var running = new Dictionary<Type, UniTask>();
        ...
        try
        {
            while (pending.Count > 0)
            {
                ...
                if (ready.Count > 0)
                {
                    ...
                    {
                        // 已达并发上限，等待至少一个完成
                        await UniTask.WhenAny(running.Values);
                        continue;
                    }
                    ...
                    for (int i = 0; i < Math.Min(slots, ready.Count); i++)
                    {
                        ...
                        running[type] = InitModuleAsync(module).Preserve();
                    }
                }
                ...
                // 等待任意一个完成
                var completed = await UniTask.WhenAny(running.Values);
                // 找到完成的 task 对应的 type
                Type completedType = null;
                foreach (var kv in running)
                {
                    if (kv.Value.Status == UniTaskStatus.Succeeded ||
                        kv.Value.Status == UniTaskStatus.Faulted ||
                        kv.Value.Status == UniTaskStatus.Canceled)
                    {
